...
public class StubGenerator implements org.hotswap.agent.javassist.Translator {
	...
	/**
	 * Initializes the object. This is a method declared in Translator.
	 *
	...
	public void start(org.hotswap.agent.javassist.ClassPool pool) throws org.hotswap.agent.javassist.NotFoundException {
		...
		proxyConstructorParamTypes = pool.get(new String[] { "ObjectImporter", "int" });
		interfacesForProxy = pool.get(new String[] { "java.io.Serializable", "Proxy" });
		exceptionForProxy = new org.hotswap.agent.javassist.CtClass[] { pool.get("RemoteException") };
	}

	/**
	 * Does nothing. This is a method declared in Translator.
	 *
	 * @see org.hotswap.agent.javassist.Translator#onLoad(org.hotswap.agent.javassist.ClassPool,
	 *      String)
	 */
	@Override
	...
	public synchronized boolean makeProxyClass(Class<?> clazz)
		...
		if (proxyClasses.get(classname) != null) {
			...
			org.hotswap.agent.javassist.CtClass ctclazz = produceProxyClass(classPool.get(classname), clazz);
	...
	private org.hotswap.agent.javassist.CtClass produceProxyClass(org.hotswap.agent.javassist.CtClass orgclass,
			Class<?> orgRtClass) throws org.hotswap.agent.javassist.CannotCompileException,
					org.hotswap.agent.javassist.NotFoundException {
		int modify = orgclass.getModifiers();
		if (org.hotswap.agent.javassist.Modifier.isAbstract(modify)
				|| org.hotswap.agent.javassist.Modifier.isNative(modify)
				|| !org.hotswap.agent.javassist.Modifier.isPublic(modify)) {
			throw new org.hotswap.agent.javassist.CannotCompileException(
					orgclass.getName() + " must be public, non-native, and non-abstract.");
		}

		org.hotswap.agent.javassist.CtClass proxy = classPool.makeClass(orgclass.getName(), orgclass.getSuperclass());

		proxy.setInterfaces(interfacesForProxy);

		org.hotswap.agent.javassist.CtField f = new org.hotswap.agent.javassist.CtField(classPool.get("ObjectImporter"),
				fieldImporter, proxy);
		...
		org.hotswap.agent.javassist.CtConstructor cons = org.hotswap.agent.javassist.CtNewConstructor
				.skeleton(proxyConstructorParamTypes, null, proxy);
	...
	private org.hotswap.agent.javassist.CtClass toCtClass(Class<?> rtclass)
	...
	private org.hotswap.agent.javassist.CtClass[] toCtClass(Class<?>[] rtclasses)
			throws org.hotswap.agent.javassist.NotFoundException {
	...
	private void addMethods(org.hotswap.agent.javassist.CtClass proxy, Method[] ms)
		...
		for (int i = 0; i < ms.length; ++i) {
			...
			if (m.getDeclaringClass() != Object.class && !org.hotswap.agent.javassist.Modifier.isFinal(mod)) {
				if (org.hotswap.agent.javassist.Modifier.isPublic(mod)) {
					...
					wmethod = org.hotswap.agent.javassist.CtNewMethod.wrapped(toCtClass(m.getReturnType()), m.getName(),
							toCtClass(m.getParameterTypes()), exceptionForProxy, body,
							org.hotswap.agent.javassist.CtMethod.ConstParameter.integer(i), proxy);
	...
	private void modifySuperclass(org.hotswap.agent.javassist.CtClass orgclass)
		...
		for (;; orgclass = superclazz) {
			...
			superclazz.addConstructor(org.hotswap.agent.javassist.CtNewConstructor.defaultConstructor(superclazz));
		}
	}
}
